...
在 DLL 中可以定义两种函数：导出函数和内部函数（exported and internal）。导出函数可以被其他模块调用，也可以被它所在的模块调用。内部函数一般而言用于 DLL 内。 DLL 的定义的数据一般只在 DLL 内使用。 
...
系统会对入口点函数进行调用，该调用在调用 `LoadLibrary` 或 `LoadLibraryEx` 的上下文中。如果 DLL 已经载入，在没有出现相对应的 `FreeLibrary` 函数的情况下，通过 `LoadLibrary` 或 `LoadibraryEx` 载入的 DLL 的入口点函数不会被调用。
...
在 DLL 中定义为全局变量的变量会被编译器和连接器作为全局变量对待，每个载入给定 DLL 的进程都会有属于它自己的 DLL 全局变量。静态变量被限制在定义它的区域内。
...
对于导出函数，如果制定了调用约定关键字，那么关键字 `__declspec(dllexport)`  必须写在调用约定关键字的左边。例如
...
创建 DLL 时，一般会创建一个包含导出的函数原型和导出的类的头文件（用于 DLL 的构建），并将在这些声明中加上 `__declspec(dllexport)` 。为了让代码的可读性更强，可以为 `__declspec(dllexport)` 定义一个宏，并使用这个宏来标注导出函数。
...
如果你需要将 C 函数供 C++ 程序使用，而函数声明头文件又没有这么做，那么可以这样做来避免 C++ 编译器对 C 函数名进行装饰：
...
- 文件的第一个语句必须是 LIBRARY 语句。这条语句标识 DEF 所属的 DLL。LIBRARY 后接 DLL 的名称。连接器会将这个名字放在 DLL 的导入库中。
...
如果 `func1` 存在于其它 DLL 中（而不是 .exe 中），链接器不能直接对其进行分析，因为它无法知道 `func1` 的地址。在 16 位环境中，链接器将地址添加在 .exe 文件的表中，载入器可以通过它在运行时使用正确的地址进行相应的修补。在 32 位或 64 位环境中，连接器会生成一个 [*thunk*](https://en.wikipedia.org/wiki/Thunk) ，它知道地址。在 32 位环境中看起来就像这样：
...
因此，使用 `__declspec(dllimport)` 更好，因为如果不需要的话，连接器就不会生成一个 thunk。Thunk 让代码更多并可能会降低 cache 性能。如果你告诉编译器该函数在 DLL 中，它能为你产生一个间接调用。
...
对数据而言，使用 `__declspec(dllimport)` 可以消除一层间接。当你从 DLL 中导出数据时，你任然需要浏览整个导入地址表，这意味着你当你访问 DLL 导出的数据时，你不得不记住额外的间接层，执行额外的间接寻址，就像这样：
...
这个动态库中包含两个函数 `add` 和 `multi` ，它们的功能分别是对两个 `int` 型整数相加，和对两个 `int` 型整数相乘，为了简便起见，要求两个数都不小于 0。它还含有一个值为 0 的变量 `yyzero` ，它的值为 0。
...
即可得到 yylib.dll 和 yydll\_dll.a 文件。.a 文件对应于 MSVC 中的 .lib 导入库。
...
`__cdecl` 是 C 和 C++ 的默认调用约定。因为栈的清理工作由调用者完成，它可以用作 `vararg` 函数。`__cdecl` 调用约定会创建比使用 `__stdcall` 更大的可执行程序，因为它需要每个函数调用包含清理栈的代码。
