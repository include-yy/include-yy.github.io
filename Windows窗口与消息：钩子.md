...
**WH\_FOREGROUNDIDLE** 钩子允许你在前台线程闲置（idle）时处理低优先级的任务。当前台线程将要闲置时，系统会调用 **WH\_FOREGROUNDIDLE** 钩子过程。
...
**WH\_MOUSE** 钩子允许你监视将要由 **GetMessage** 或 **PeekMessage** 返回的鼠标消息。你可以使用它来监视投递到消息队列的鼠标输入。
